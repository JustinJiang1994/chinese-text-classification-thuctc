...
### 模型特点分析
1. **表现最好的类别**: 房产、汽车、娱乐、国际类别表现较好，F1分数均超过75%
2. **表现较差的类别**: 财经、社会、家居类别表现较差，F1分数低于60%
3. **类别区分度**: 房产和汽车类别具有较好的区分度，可能与这些类别的专业术语较多有关
4. **混淆情况**: 社会、财经、家居类别容易混淆，可能与这些类别的通用词汇较多有关

### 测试示例
模型在测试文本上的表现：
- "特斯拉发布新款电动汽车" → 汽车类别 (置信度92.97%)
- "NBA季后赛激战正酣" → 体育类别 (置信度94.09%)
- "房地产市场调控政策" → 房产类别 (置信度99.21%)
- "好莱坞明星获得奥斯卡奖" → 娱乐类别 (置信度89.83%)
